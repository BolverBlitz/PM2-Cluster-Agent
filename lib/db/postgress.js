...
/**
 * This function will create or update a process
 * @returns {Promise}
 */
 ...
 */
 const GetAllProcesses = function() {
  return new Promise(function(resolve, reject) {
    pool.query(`SELECT * FROM public.processes WHERE updated_at > now()::timestamp - interval '91h' ORDER BY server ASC, pm2id ASC`, (err, result) => {
      if (err) {reject(err)}
        resolve(result);
    });
  });
